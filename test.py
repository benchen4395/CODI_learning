...
def evaluation(model_args, data_args, training_args):
    ...
    model.codi.tie_weights()    # 输入/输出层权重共享的关键方法，通过减少参数冗余提升模型效率; 通常是输入和输出的嵌入层（embedding layers）共享参数
    ...
    if tokenizer.pad_token_id is None:
        ...
        if tokenizer.pad_token_id is None: # error handling
            tokenizer.pad_token_id = tokenizer.convert_tokens_to_ids('[PAD]')   # gpt-2对所有不存在vocab的词，都映射成50256，即'<|endoftext|>'
    ...
    for i in range(eval_step):
        if i < eval_step - 1:   # batch_size = 128
            batch = tokenizer(
            ...
            batch = tokenizer(
                ...
                return_tensors="pt",
                padding="longest",  # 以最长文本的长度编码, 而不是默认的1024
            )
    ...
    for step, batch in enumerate(question_data):
        ...
        with torch.no_grad():
            ...
            latent_embd = outputs.hidden_states[-1][:, -1, :].unsqueeze(1)  # 最后一层transformers层的输出[batch_size, 1, hidden_size]

            if training_args.use_prj:   # test时, 是做了两层线性层, [hidden_size -> prj_size -> hidden_size]
            ...
            for i in range(gen_kwargs["max_new_tokens"]):       # 256
                ...
                if training_args.greedy:    # True
                    next_token_ids = torch.argmax(logits, dim=-1).squeeze(-1)
                else:
                    logits /= gen_kwargs["temperature"] # 0.1
                ...
                if finished.all():
                    print('_________________the finished is all true!')
                    break
