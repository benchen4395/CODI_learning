...
class TrainingArguments(transformers.TrainingArguments):
    ...
    ref_loss_factor: float = field(default=1.0, metadata={"help": "A multiplier that scales the teacher's cross entropy loss in the total loss calculation.."})
    ...
    print_loss: bool = field(default=True, metadata={"help": "Whether print the loss."})
...
class CODI(torch.nn.Module):
    def __init__(self, model_args, training_args, lora_config):
        ...
        if model_args.full_precision:       # 是否开启int4量化
            ...
            # 这个配置要求: 必须支持 bfloat16（Ampere+架构：RTX 30系/A100+）\ CUDA >= 11.0
            self.codi = AutoModelForCausalLM.from_pretrained(
                    self.model_name,
        ...
        self.training = self.model_args.train       # 判断是训练，还是测试

        # special tokens to enclose the latent embeddings
        self.pad_token_id = ori_vocab_size  # 50257: 0-50256
        self.bot_id = ori_vocab_size + 1    # 50258
        self.eot_id = ori_vocab_size + 2    # 50259

        # gpt2: special_tokens={'bos_token': '<|endoftext|>', 'eos_token': '<|endoftext|>', 'unk_token': '<|endoftext|>'}
        ...
        # Projection Layer
        self.use_prj = training_args.use_prj    # Whether use a projection layer for the last layer hidden state
        self.prj_no_ln = training_args.prj_no_ln    # Whether the projection layer is not followed by a LayerNorm layer.
        # 确定是否对最后一层增加两层线性层，同时是否增加LayerNorm
        if training_args.use_prj:
            self.prj = nn.Sequential(
                nn.Dropout(training_args.prj_dropout),      # test时为0
                nn.Linear(self.dim, training_args.prj_dim), # test时为768, 
                nn.GELU(),
                nn.Linear(training_args.prj_dim, self.dim), # test是为1024,
            )
        ...
        self.distill_loss_factor = training_args.distill_loss_factor    # 1.0
        ...
        # general # Correct a bug about attention mask.
